...
class Jogo:
	...
	#Verifica se os movimentos são validos
...
#Função que os botões aparecer na tela para o usuario
def cria_botao(msg, sqr, cor1, cor2, cor_texto, acao=None):
	...
	fontePequena = pygame.font.SysFont('Jogo Damas Matheus', 20)
...
def tela_vencedor(vencedor):
	...
	while not sair:
		...
		fonte = pygame.font.SysFont('Jogo Damas Matheus.', 50)
		...
		voltar = fonte.render('Pressione qualquer tecla para retornar ao inicio.', False, VERDE_CLARO)
...
def menu_jogo():
	while True:
		...
		display.fill(PRETO)
		fonte = pygame.font.SysFont('Jogo de Damas Matheus.', 50)
		surface_texto, rect_texto = text_objects("Jogo de Damas Matheus.", fonte, BRANCO)
		rect_texto.center = ((LARGURA / 2), ALTURA / 3)
		display.blit(surface_texto, rect_texto)

		cria_botao("Iniciar",(LARGURA - 560, ALTURA / 2, 120, 40), BRANCO, CINZA, PRETO, loop_jogo)
		cria_botao("Sair",(LARGURA - 360, ALTURA / 2, 120, 40), BRANCO, CINZA, PRETO, sair)
